...
class Knockout{
    ...
    reguliere_speeltijd(land1,land2){
        ...
        for(let el of land1.toUpperCase().replaceAll(" ","")){
            if(hoofdstad2.includes(el)){
                hoofdstad2=hoofdstad2.replace(el,"");
                score1++;
            }
        }
        for(let i of land2.toUpperCase().replaceAll(" ","")){
    ...
    wedstrijd(land1,land2){
        let landen=[land1,land2];
        let normal=this.reguliere_speeltijd(land1,land2);
        if(normal[0]===normal[1]){
            let special=this.extra_speeltijd(land1,land2);
