...
const richtingen = { N: [-1, 0], O: [0, 1], Z: [1, 0], W: [0, -1] }
const tekentjes = { ZO: '┗', WN: '┗', ZZ: '┃', NN: '┃', ZW: '┛', ON: '┛', WZ: '┏', NO: '┏', OZ: '┓', NW: '┓', OO: '━', WW: '━' }
class Puzzel {
  constructor (rijwaarden, kolomwaarden, ingang, uitgang) {
    if ((ingang[0] === uitgang[0] && ingang[1] === uitgang[1]) || !(ingang[0] % (rijwaarden.length - 1) === 0 || ingang[1] % (kolomwaarden.length - 1) === 0) || !(uitgang[0] % (rijwaarden.length - 1) === 0 || uitgang[1] % (kolomwaarden.length - 1) === 0) ||
            !(rijwaarden.every(el => el >= 0 && el <= kolomwaarden.length) && kolomwaarden.every(el => el >= 0 && el <= rijwaarden.length))) {
  ...
  isBinnen (pos) {
    return pos[0] >= 0 && pos[1] >= 0 && pos[1] <= this.kolomwaarden.length && pos[0] <= this.rijwaarden.length
  }
  ...
  stap (richting) {
    assert(richting in richtingen, 'ongeldige richting')
    const [r, k] = this.positie
    const [dx, dy] = richtingen[richting]
    const newPos = [r + dx, k + dy]
    const cpos = this.bord[newPos[0]]?.[newPos[1]]
    ...
    if (this.vorige) this.bord[r][k] = tekentjes[this.vorige + richting]
  ...
  toString () {
    const copy = this.bord.map(el => [...el])
